...
def main(_run, _config, _log):
    ...
    dataset = exp_disaster_fewshot(
        images_dir=_config["path"]["ExpDisaster"]["meta_train_images"],
        labels_dir=_config["path"]["ExpDisaster"]["meta_train_labels"],
        class_remap=_config["exp_disaster"]["train"]["class_remap"],
        n_ways=_config["task"]["n_ways"],
        n_shots=_config["task"]["n_shots"],
        n_queries=_config["task"]["n_queries"],
        max_iters=_config["n_steps"] * _config["batch_size"],
        ignore_label=_config["ignore_label"],
        allowed_classes=_config["exp_disaster"]["train"]["allowed_classes"],
        seed=_config["seed"],
    )
    trainloader = DataLoader(
        dataset, batch_size=_config["batch_size"], shuffle=True, num_workers=1, pin_memory=True, drop_last=True
    )
